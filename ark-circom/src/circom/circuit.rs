...
use ark_relations::r1cs::{
    ConstraintSynthesizer, ConstraintSynthesizerAsync, ConstraintSystemRef,
    ConstraintSystemRefAsync, LinearCombination, SynthesisError, Variable,
};
    ...
    pub fn get_public_inputs(&self) -> Option<Vec<E::ScalarField>> {
        match &self.witness {
            ...
            Some(w) => match &self.r1cs.wire_mapping {
                ...
                Some(m) => Some(
                    m[1..self.r1cs.num_inputs].iter().map(|i| w[*i]).collect(),
                ),
            },
        }
    }
...
impl<E: Pairing> ConstraintSynthesizerAsync<E::ScalarField>
    for CircomCircuit<E>
{
    async fn generate_constraints_async(
        self,
        cs: ConstraintSystemRefAsync<E::ScalarField>,
    ) -> Result<(), SynthesisError> {
        let witness = &self.witness;
        let wire_mapping = &self.r1cs.wire_mapping;
